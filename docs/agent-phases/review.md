...
Report `APPROVED` or `CHANGES_REQUESTED`, the iteration number and whether it's at or past the cap, plus the list of blocking findings (file:line + one-liner each).
